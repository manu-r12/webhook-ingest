...
func TestCacheRecordConcurrentRace(t *testing.T) {
	c := stats.NewCache()
	...
	got := c.Get("acc_race")
	if got.CallCount != 100 || got.TotalDurationSec != 1000 {
		t.Fatalf("got %+v, want CallCount=100 TotalDurationSec=1000", got)
	}
}
